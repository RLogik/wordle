...
fn compare_guess(guess: &str, solution: &str) -> WordleState {
    let guess_chars = utils::chars_to_uppercase(utils::string_to_chars(guess));
    ...
    return WordleState::new(guess, &feedback.as_str());
}
...
pub fn change_distance(guess: &str, solution: &str) -> (i32, i32) {
    let wordl_state = WordleState::from(guess, solution);
