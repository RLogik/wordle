...
/// ## Parts ##
///
/// - `state` - says whether input state is complete/pending/meta+d(=>cancel)/meta+c(=>quit).
/// - `value` - value type `T`.
#[derive(Debug)]
...
/// ## Parts ##
///
/// - `state` - type `ResponseState`.
/// - `symbols` - vector of strings.
/// - `cursor` - tracks position of cursor.
///
/// ## Methods/Examples ##
///
/// ```rust
/// use wordle::models::console::ConsoleTextState;
/// use wordle::models::response::ResponseState;
/// let mut response = ConsoleTextState::new();
/// assert_ne!(response.state, ResponseState::CANCEL);
/// assert_ne!(response.state, ResponseState::QUIT);
...
impl ConsoleTextState {
    pub fn new() -> Self {
        ...
        let cursor: usize = 0;
        return ConsoleTextState {symbols, cursor, state: ResponseState::PENDING};
    }
...
///
/// ```rust
/// use wordle::models::console::interaction;
/// use wordle::models::response::ResponseState;
            ...
            Ok(e) => {
                match e {
                    ...
                    => {
                        ...
                        response.state = ResponseState::QUIT;
                        break;
                    },
                    // META + D ==> 'cancel' state:
                    ...
                    => {
                        response = ConsoleTextState::new();
                        response.state = ResponseState::CANCEL;
                        break;
                    },
